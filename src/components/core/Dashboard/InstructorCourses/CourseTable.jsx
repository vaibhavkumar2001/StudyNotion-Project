      ...
      <Table className="rounded-xl border border-richblack-800">
        ...
        <Tbody>
          {courses?.length === 0 ? (
            ...
            courses?.map((course) => (
              <Tr key={course?._id || Math.random()} className="flex gap-x-10 border-b border-richblack-800 px-6 py-8">
                <Td className="flex flex-1 gap-x-4">
                  <img src={course?.thumbnail} alt={course?.courseName} className="h-[148px] w-[220px] rounded-lg object-cover" />
                  <div className="flex flex-col justify-between">
                    <p className="text-lg font-semibold text-richblack-5">{course?.courseName || "Untitled"}</p>
                ...
                <Td className="text-sm font-medium text-richblack-100">₹{course?.price ?? "N/A"}</Td>
